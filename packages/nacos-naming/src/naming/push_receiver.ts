...
import { tryDecompress } from '../utils';

export class PushReceiver extends Base {
  private _udpPort: number;
  private _inited: boolean;
  private _isClosed: boolean;
  private _hostReactor: any;
  private _server: any;
  ...
  _createServer(): void {
    ...
    this._server.once('error', (err: any) => {
      this._server.close();
    ...
    this._server.on('message', (msg: any, rinfo: any) => this._handlePushMessage(msg, rinfo));
  ...
  _handlePushMessage(msg: any, rinfo: any): void {
    try {
      const jsonStr = tryDecompress(msg).toString();
      ...
      (err as any).name = 'PushReceiverError';
      (err as any).message += ' error while receiving push data';
      this.emit('error', err);
    }
  }
